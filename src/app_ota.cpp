...
void ota_task(void *args)
{
  ...
  Preferences Mem;
  Mem.begin("CNFG", true);
  bool wlan = Mem.getBool("wlan", false);

  if (wlan)
  {
    Serial.printf("SSID <%s> PSWD <%s>\r\n", Mem.getString("SSID").c_str(), Mem.getString("PSWD").c_str());
    WiFi.begin(Mem.getString("SSID").c_str(), Mem.getString("PSWD").c_str());
  ...
  {
    DEBUG_e("[WLAN NOT SAVED]");
    vTaskDelete(NULL);
  }
  Mem.end();

  DEBUG_i("CALLING HTTP REQUEST");

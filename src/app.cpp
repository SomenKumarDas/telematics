...
void SEND_CAN_RESPONSE(void)
{
  ...
  if (CanIsoRcv.newMessage)
  {
    ...
    else if (PROTOCOL == APP_CAN_PROTOCOL_OE_IVN)
    {
      ...
      value.U32 = CanIsoRcv.RxID;
      IVNbuff[0] = value.Byte.B_3;
      IVNbuff[1] = value.Byte.B_2;
      IVNbuff[2] = value.Byte.B_1;
      IVNbuff[3] = value.Byte.B_0;
      memcpy(&IVNbuff[4], CanIsoRcv.SF_buf, CanIsoRcv.RxMsgLen);

      crc16 = UTIL_CRC16_CCITT(0xFFFF, IVNbuff, 12);
      for (uint16_t i = 12; i > 0; i--)
      {
        IVNbuff[i + 1] = IVNbuff[i - 1];
      }
      IVNbuff[respLen++] = 0x60 | (((12 + 2) >> 8) & 0x0F);
      IVNbuff[respLen++] = (12 + 2);
      respLen = respLen + 12;
 ...
 * FRAME ID -> 4
 * ISO_CAN_FRAME
 * Send the data to can with ISO 15765 implemented
 */
static void ISO_CAN_FRAME(uint8_t *p_buff, uint16_t len, uint8_t channel)
{
  if (len <= 4096)

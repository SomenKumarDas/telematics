...
void Send_TTP(void)
{
  if ((IsTimerElapsed(APP_TstrPrTmr)) && (APP_CAN_TransmitTstrMsg))
  {
    ...
    memset(tx_frame.data.u8, frame.Padding, 8);
...
void RCV_ISO(void)
{
  if (xQueueReceive(CAN_cfg.rx_queue, &rx_frame, pdMS_TO_TICKS(5)) == pdTRUE)
  {
    ...
    else if (PROTOCOL == APP_CAN_PROTOCOL_OE_IVN)
    {
      ...
      memcpy(CanIsoRcv.SF_buf, rx_frame.data.u8, CanIsoRcv.RxMsgLen);
...
static void HandleRX_FF()
{
  ...
  // tx_frame.MsgID = frame.TxID;
  // tx_frame.FIR.B.FF = (CAN_frame_format_t)frame.TxIdExD;
  tx_frame.FIR.B.DLC = 8;
  tx_frame.data.u8[0] = 0x30;
  tx_frame.data.u8[1] = 0;
  tx_frame.data.u8[2] = 0xF9;
  memset(&tx_frame.data.u8[3], frame.Padding, 5);
...
static void HandleRX_FC()
{
  ...
  //   Serial.printf("FC_BS: %02x", frame.FC_BS);
  // }
  // frame.Tx_CF_Idx = 1;
  ...
  while (frame.SetTx_FF)
  {
    ...
    if ((tx_frame.FIR.B.DLC < 8))
    {
      memset((&tx_frame.data.u8[0] + tx_frame.FIR.B.DLC),
             (uint8_t)frame.Padding,
    ...
    // if(frame.Tx_CF_Idx == frame.FC_BS)
    // {
    //   break;
    // }
    frame.Tx_CF_Idx++;
